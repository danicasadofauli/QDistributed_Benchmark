...
from .utils.arithmetic import is_power
...
class Shor:
    ...
    def construct_circuit(self, N: int, circuit: QuantumCircuit, a: int = 2, measurement: bool = False) -> QuantumCircuit:
        ...
        up_qreg = [i for i in range(2*n)] #QuantumRegister(2 * n, name="up")
        # quantum register where the multiplications are made
        down_qreg = [i for i in range(2*n, 3*n)] #QuantumRegister(n, name="down")
        # auxiliary quantum register used in addition and multiplication
        #aux_qreg = [i for i in range(3*n, 4*n + 2)] # QuantumRegister(n + 2, name="aux")
        ...
        # Count communication qubits
        #no_com_qubits = [qubit for qubit in circuit.qubits if "com_qnode" not in qubit._register._name]

        # Apply modulo exponentiation
        modulo_power = self._power_mod_N(n, N, a)

        circuit.append(modulo_power, circuit.qubits[:4*n + 2])
