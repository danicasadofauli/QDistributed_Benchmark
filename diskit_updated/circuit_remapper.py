...
from diskit.components import Layer, Topology
...
class CircuitRemapper:
    ...
    def _layer_to_circuit(layers: Iterable[List], qubits: Iterable[Qubit] = (),
        ...
        if qubits:
            ...
            added_qregs.update(qubit._register for qubit in qubits)
        ...
        if clbits:
            ...
            added_cregs.update(clbit._register for clbit in clbits)
        ...
        for layer in layers:
            for instruction in layer:
                ...
                qubits = [qubit for qubit in instruction.qubits if
                          qubit not in added_qubits and qubit._register not in added_qregs]
                clbits = [clbit for clbit in instruction.clbits if
                          clbit not in added_clbits and clbit._register not in added_cregs]
                qregs = [qubit._register for qubit in qubits]
                cregs = [clbit._register for clbit in clbits]
                circuit.add_bits(qubits)
                circuit.add_bits(clbits)
    ...
    def _replace_nonlocal_control(operation: CircuitInstruction,
        ...
        ent_inst = ent_inst.replace(qubits = [epr_qubits[1], opr_qubits[1]])
